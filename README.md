...
- 小米风格网页选择面板，Android / iOS 交互一致，不再调用系统原生选择页
- 小米风格操作确认弹窗，替代 Android / iOS 原生确认框
...
也可以把整个 `mihomo-plugin` 目录复制到小米智能存储，然后在设备的 root SSH 终端内直接运行同一条命令。脚本会识别本机环境并直接安装。
...
`uninstall.sh` 同样支持在小米智能存储的 root SSH 终端内直接运行。
